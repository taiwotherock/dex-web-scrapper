...
const save_coingecko_1 = require("./save-coingecko");
async function run() {
    //connect the existing browser using Websocket debugger address
    const browser = await playwright_1.chromium.connectOverCDP('ws://127.0.0.1:9242/devtools/browser/6ac044a8-867a-4fb1-bf4e-03c5297a94ff');
    ...
    for (i = 0; i < count; i++) {
        ...
        var namex = name ?? '';
        var img = imgUrl ?? '';
        (0, save_coingecko_1.insertData)(namex, namex, '', img);
    }
    browser.close();
}
run().catch((err) => {
