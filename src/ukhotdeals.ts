...
const browser = await chromium.connectOverCDP('ws://127.0.0.1:9242/devtools/browser/4917412c-6280-4e5d-9765-07c4718dd542');
...
await page.goto('https://www.hotukdeals.com/hottest'); ///solana/raydium')
...
let table = page.locator("div.listLayout-main"); //threadListCard
await table.waitFor();
  console.log("data " + table.innerHTML() + " " + table.innerText())
let rows = await table.locator("div.threadListCard")
...
let i =0;
  ...
  for (i=0; i<count; i++)
  {
      ...
      try {
      const imgUrl = await rows.nth(i).locator(".imgFrame").first().locator("img").getAttribute("src")
      const name = await rows.nth(i).locator(".imgFrame").first().locator("img").getAttribute("alt")
      const price = await rows.nth(i).locator(".thread-price").innerText()
      const details = await rows.nth(i).locator(".userHtml").first().innerText()
